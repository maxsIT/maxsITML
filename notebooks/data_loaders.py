...
def load_faculty_data (
    path_fict = '../input/FICT.txt',
    path_fem = '../input/FEM.txt',
    path_fim = '../input/FIM.txt',
    path_gef = '../input/GEF.txt'):
    """
    ...
    fict = get_faculty_members(path_fict)
    fem = get_faculty_members(path_fem)
    fim = get_faculty_members(path_fim)
    gef = get_faculty_members(path_gef)
    
    return fict, fem, fim, gef

def parse_line_to_fict_df (line):
    """
...
def load_processed_data ():
    ...
    fict, fem, fim, gef = load_faculty_data()
    persons = []
    for person in fict:
        featurized_person = parse_line_to_fict_df(person)
        featurized_person['is_fict'] = 1
        featurized_person['is_fem'] = 0
        featurized_person['is_fim'] = 0
        featurized_person['is_gef'] = 0
        persons.append(featurized_person)
    for person in fem:
        featurized_person = parse_line_to_fict_df(person)
        featurized_person['is_fict'] = 0
        featurized_person['is_fem'] = 1
        featurized_person['is_fim'] = 0
        featurized_person['is_gef'] = 0
        persons.append(featurized_person)
    for person in fim:
    ...
    for person in gef:
        ...
        featurized_person['is_fem'] = 0
        featurized_person['is_fim'] = 0
        featurized_person['is_gef'] = 1
        persons.append(featurized_person)
    

    fict_df = pd.DataFrame(persons,
        columns=[
         'name', 
         'surname', 
         'is_english',
